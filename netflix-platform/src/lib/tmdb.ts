...
export async function getPopular(mediaType: "movie" | "tv" = "movie", page = 1): Promise<TMDBMovie[]> {
  const data = await fetchTMDB(`/${mediaType}/popular`, { page: String(page) });
  return data.results;
}

export async function getTopRated(mediaType: "movie" | "tv" = "movie", page = 1): Promise<TMDBMovie[]> {
  const data = await fetchTMDB(`/${mediaType}/top_rated`, { page: String(page) });
  return data.results;
}
...
export async function getByGenre(genreId: number, mediaType: "movie" | "tv" = "movie"): Promise<TMDBMovie[]> {
...
export const GENRE_MAP: Record<number, string> = {
  ...
  27: "رعب", 10402: "موسيقي", 9648: "غموض", 10749: "رومانسي", 878: "خيال علمي",
