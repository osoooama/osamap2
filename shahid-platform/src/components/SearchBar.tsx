  ...
  return (
    <div className={cn("relative", className)} dir="rtl">
      <button
        ...
        className="text-zinc-400 hover:text-[#C9A96E] transition-colors p-2"
      >
          ...
          >
            <div className="relative flex items-center">
              <input
                ...
                className="w-56 sm:w-72 bg-[#0D3320]/90 border border-[#C9A96E]/20 text-white text-sm rounded-lg pl-9 pr-10 py-2 focus:outline-none focus:border-[#C9A96E]/50 placeholder:text-zinc-500 backdrop-blur-sm"
              />
              <Search className="absolute right-3 w-4 h-4 text-zinc-500 pointer-events-none" />
              {query && (
                <button
                  onClick={() => setQuery("")}
                  className="absolute left-3 text-zinc-500 hover:text-white"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
            ...
            {query && results.length > 0 && (
              <div className="absolute top-full right-0 mt-2 w-80 sm:w-96 bg-[#0A2818] border border-[#C9A96E]/10 rounded-xl overflow-hidden shadow-2xl shadow-black/50 z-50">
                {loading ? (
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {results.map((movie) => (
                      <div
                        key={movie.id}
                        className="flex items-center gap-3 p-3 hover:bg-[#0D3320]/50 transition-colors cursor-pointer"
                        onClick={() => {
                          onInfo?.(movie);
                          setIsOpen(false);
                          setQuery("");
                      ...
                      >
                        <div className="w-10 h-14 rounded overflow-hidden bg-[#0D3320] flex-shrink-0">
                          {movie.poster_path ? (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-xs text-zinc-500">
                            ...
                            {movie.vote_average > 0 && (
                              <span className="text-[#C9A96E]">{getMatchPercent(movie.vote_average)}%</span>
                            )}
                        ...
                        <button
                          ...
                          className="w-8 h-8 rounded-full bg-[#C9A96E]/10 flex items-center justify-center hover:bg-[#C9A96E]/20 transition-colors"
                        >
            ...
            {query && !loading && results.length === 0 && (
              <div className="absolute top-full right-0 mt-2 w-80 bg-[#0A2818] border border-[#C9A96E]/10 rounded-xl p-4 text-center shadow-2xl">
