...
import { ArrowLeft, Star, Calendar, Clock, ThumbsUp, Film } from 'lucide-react';
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-8 lg:hidden">
              <ContentInfo movie={movie} anime={anime} error={error} isAnime={isAnime} />
            </div>
          </div>
...
function ContentInfo({ movie, anime, error, isAnime }: { movie: any; anime: AnimeEntry | null; error: boolean; isAnime: boolean }) {
  if (error) {
    return (
  ...
  if (isAnime && anime) {
    return (
      <div className="space-y-5">
        <h1 className="text-2xl font-black text-white leading-tight">{anime.title}</h1>
        {anime.title_japanese && <p className="text-sm text-zinc-500">{anime.title_japanese}</p>}
