...
export default function Card({ movie, onInfo, onPlay }: CardProps) {
  ...
  return (
    <div
      ...
      dir="rtl"
    >
      <div className="relative aspect-[2/3] rounded-lg sm:rounded-xl overflow-hidden bg-[#0D3320] shadow-lg shadow-black/20 transition-all duration-500 group-hover:shadow-[0_8px_40px_-8px_rgba(0,0,0,0.8)] group-hover:scale-[1.05] group-hover:-translate-y-1">
        {posterUrl && !imgError ? (
          <>
            {!imgLoaded && (
              <div className="absolute inset-0 bg-[#0D3320] animate-pulse">
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#0D3320] to-[#0A2818]">
            <span className="text-[#C9A96E]/30 text-3xl sm:text-4xl font-black">{title[0]}</span>
          </div>
        )}
        ...
        >
          <div className="flex gap-1.5 mb-1.5">
            <button
              ...
              className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-xl bg-[#C9A96E]"
            >
              <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#0A2818] fill-[#0A2818] mr-0.5" />
            </button>
            <button
              onClick={(e) => {
                e.stopPropagation();
        ...
        <div className="absolute top-1.5 sm:top-2 right-1.5 sm:right-2 flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md sm:rounded-lg bg-black/70 backdrop-blur-md text-[10px] sm:text-xs border border-white/10">
          <span className="text-[#C9A96E] font-bold">{rating}%</span>
        </div>

        <div className="absolute top-1.5 sm:top-2 left-1.5 sm:left-2 flex gap-1">
          {mediaType === "tv" && (
            <div className="px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md sm:rounded-lg bg-[#0A2818]/80 backdrop-blur-md text-[10px] sm:text-xs text-[#C9A96E] font-semibold border border-[#C9A96E]/20 flex items-center gap-0.5">
      ...
      <div className="mt-1.5 sm:mt-2 px-0.5 space-y-0.5" dir="auto">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-zinc-500 flex-wrap">
          ...
          {rating > 0 && (
            <span className="flex items-center gap-0.5 text-[#C9A96E] font-semibold">{rating}%</span>
          )}
        </div>
        {genreNames && (
      ...
      <AnimatePresence>
        {showPopup && !isMobile.current && (
          <motion.div
            ...
            className="w-[280px] sm:w-[300px] rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl shadow-black/70 border border-[#C9A96E]/10 pointer-events-auto"
          ...
          >
            <div className="relative aspect-video">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#060F0A]/90 via-[#060F0A]/30 to-transparent" />
              <div className="absolute bottom-2.5 left-2.5 right-2.5 flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl bg-[#C9A96E] text-[#0A2818] font-bold text-xs sm:text-sm hover:bg-[#D4B87A] transition shadow-lg"
                >
                  <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-[#0A2818]" />
                  مشاهدة
                </button>
                <button
                  onClick={toggleFav}
            ...
            <div className="p-2.5 sm:p-3 bg-[#0A2818] space-y-1.5">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 text-[9px] sm:text-[10px] text-zinc-400">
                {year && <span>{year}</span>}
                {rating > 0 && (
                  <span className="flex items-center gap-0.5 text-[#C9A96E] font-semibold">
                    {rating}% توافق
                  </span>
                )}
