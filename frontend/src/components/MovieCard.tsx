...
export default function MovieCard({ movie }: MovieCardProps) {
  ...
  return (
    <div
      className="group relative flex-shrink-0 cursor-pointer transition duration-300 ease-in-out hover:scale-105 hover:drop-shadow-lg"
      onClick={() => router.push(`/player?tmdb_id=${movie.tmdb_id}`)}
    >
      <div className="relative h-56 w-40 overflow-hidden rounded-md sm:w-48 lg:w-56 lg:h-72">
        <Image
          src={imgSrc}
          alt={movie.title}
          fill
