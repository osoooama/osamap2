    ...
    for domain in ANIMESLAYER_DOMAINS:
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f'[ANIMESLAYER] Domain {domain} -> {final_domain}')
                return f'https://{final_domain}'
        except Exception:
            pass
    return 'https://animeslayer.to'
        ...
        with sync_playwright() as p:
            ...
            for slug in POPULAR_SLUGS:
                ...
                try:
                    ...
                    cards = page.query_selector_all('div.ep-card, [data-href], .episode-card, .ep-item, .episode')
                    ...
                    for card in cards[:3]:
                        ...
                        if a_el:
                            watch_url = a_el.get_attribute('href') or ''

                        if not watch_url:
                            continue

                        if not watch_url.startswith('http'):
                            watch_url = f'{base_url}{watch_url}'
